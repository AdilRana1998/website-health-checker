...
const isEncryptionEnabled =
	process?.env?.DATA_ENCRYPTION_ENABLED == 'true' ?? false;
const sensitiveDataKey =
	process?.env?.SENSITIVE_DATA_KEY ?? 'sensitive_data_key';
const initializationVector =
	process?.env?.ENCRYPTION_IV ?? 'initialization_vector';
...
Encryptions.responseDecryption = function (encrypted) {
	...
	// 	try {
	// 		console.log("encrypted.request", encrypted.request);
	// 		var decrypted = CryptoJS.AES.decrypt(encrypted.request, cardEncryption);
	// 		return JSON.parse(decrypted.toString(CryptoJS.enc.Utf8));
	// 	} catch (error) {
	// 		console.log("Error while parsing decrption", error);
	// 		return ;
	// 	}
	// } else {
	// 	return encrypted;
...
Encryptions.responseEncryption = function (encrypted) {
	// if (process.env.DATA_ENCRYPTION_ENABLED == 'true') {
	// 	const response = {
	// 		response: CryptoJS.AES.encrypt(JSON.stringify(encrypted), process.env.ENCRYPTION_KEY)
	// 	}
	// 	return response;
	// }
	// else {
...
Encryptions.sha256 = async (ascii) => {
	...
	Sha256.hash = function (msg, utf8encode) {
		...
		for (var i = 0; i < N; i++) {
			...
			for (var t = 0; t < 64; t++) {
				var T1 =
					h + Sha256.Sigma1(e) + Sha256.Ch(e, f, g) + K[t] + W[t];
	...
	return await Sha256.hash(ascii);
};

Encryptions.initKey = (encryptionKey) => {
	const key = CryptoJS.SHA256(encryptionKey);
	return CryptoJS.enc.Hex.parse(key.toString());
};

Encryptions.encryptString = (data, secretKey) => {
	const key = Encryptions.initKey(secretKey);
...
Encryptions.decryptString = (encryptedData, secretKey) => {
	const key = Encryptions.initKey(secretKey);
...
Encryptions.encryptSensitiveData = (data = '') => {
	if (!data) return data;
	try {
		if (isEncryptionEnabled) {
			const a = Encryptions.encryptString(data, sensitiveDataKey);
			return a;
		} else {
			return data;
		}
...
function encryptString(str) {
    ...
    const encryptedChars = charCodes.map(code => code + 1); // Shift each character code by 1
    const encryptedStr = String.fromCharCode(...encryptedChars);
    return encryptedStr;
}

Encryptions.encryptFileName = (fileName) => {
    // Find the position of the last dot to separate extension
    const lastDotIndex = fileName.lastIndexOf('.');
    
    if (lastDotIndex === -1) {
        // No extension found, cannot encrypt
        return fileName;
    }
    
    // Extract filename (excluding extension) and extension
    const extension = fileName.slice(lastDotIndex); // including the dot
    const filenameWithoutExtension = fileName.slice(0, lastDotIndex);
    
    // Encrypt the filename part (excluding extension)
    const encryptedFilename = encryptString(filenameWithoutExtension);
    
    // Combine the encrypted filename with the original extension
    const encryptedFileNameWithExtension = encryptedFilename + extension;
    
    return encryptedFileNameWithExtension;
}



Encryptions.decryptSensitiveData = (encryptedData = '') => {
	if (!encryptedData) return encryptedData;
	try {
		if (isEncryptionEnabled) {
			return Encryptions.decryptString(encryptedData, sensitiveDataKey);
		} else {
